...
const placeOrder = async (order_id, public_list, private_list) => {
	const url = `${backendDomain}/orders/${order_id}`;
	const payload = {
...
const AddOrderScreen = ({ navigation, route }) => {
	...
	useEffect(() => {
		fetchOrders(orderDetail._id)
			.then((fetchedOrderData) => {
				setOriginalOrderData(fetchedOrderData);
	...
	const onSubmitOrderDetail = ({ quantity, unit, inPrivateList }) => {
		...
		if (!inPrivateList) {
			...
			console.log({ userOrderPublicList });
			...
			setOrderData((currentOrderData) => {
				...
				if (
					...
					let oq = 0;
					if (
						publicOrder.product_id in
						originalOrderData.public_products
					) {
						oq =
							originalOrderData.public_products[
								publicOrder.product_id
							].orders[unit.unit_quantity];
						if (oq) oq = oq.quantity;
						else oq = 0;
					}
					newOrderData = {
						...currentOrderData,
				...
				newOrderData.public_list_total =
					originalOrderData.public_list_total + publicListAmount;
				return newOrderData;
			});
		} else {
			const privateOrder = {
			...
			setOrderData((currentOrderData) => ({
				...currentOrderData,
				private_list_total:
					originalOrderData.private_list_total + privateListAmount,
			}));
		}
		closeAddOrderModal();
	};
	...
	const onPlaceOrder = () => {
		setLoading(true);
		placeOrder(orderDetail._id, userOrderPublicList, userOrderPrivateList)
			.then((response_data) => {
				setLoading(false);
